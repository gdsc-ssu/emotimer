...
public class OAuth2Service {
	...
	@Transactional
	public TokenResponse generateToken(User user, String email, RoleType kakao) throws IOException {
	...
	/**
	 * 소셜로그인 사용자들을 강제 회원가입한다.<br/> 이전에 가입된 회원이라면(이메일로 회원 조회시 값 존재) updateUser 로 소셜의 사용자 정보를 업데이트하여 저장한다.<br/> 만약 이전에 가입된 회원이지만(이메일로 회원 조회시 값 존재), 이전에 가입했던 소셜 벤더가 아니라면 OAuthProviderMissMatchException 익셉션을 던진다.<br/>
	 */
	@Transactional
	...
	/**
	 * 중복 가입을 막기 위한 검증을 한다. 현재 소셜 벤더와 로그인을 시도하는 유저의 이메일로 User 테이블의 벤더사를 비교하여 일치 여부를 반환한다.
	 *
