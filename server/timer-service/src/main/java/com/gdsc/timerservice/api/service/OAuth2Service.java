...
public class OAuth2Service {
     ...
     */
    public User join(String email, ProviderType providerType){
        ...
        if (existingUser != null){ // 이미 가입된 유저(이메일)라면
            ...
            existingUser = createOAuthUser(email, providerType);
        }
    ...
    private User createOAuthUser(String email, ProviderType providerType){
        User.builder()
                .email(email)
                .password(UUID.randomUUID().toString()).build(); // 비밀번호는 일단 그냥 UUID 로 하겠음.
        return null;
    }
